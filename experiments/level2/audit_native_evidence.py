...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resnet-per-class", type=Path)
    ...
    parser.add_argument("--resnet-kernels", type=Path)
    ...
    parser.add_argument("--min-resnet-images", type=int, default=500)
    parser.add_argument("--min-qwen-agreement", type=float)
    parser.add_argument("--max-qwen-perplexity-ratio", type=float)
    parser.add_argument("--min-resnet-logit-cosine", type=float)
    parser.add_argument("--min-resnet-reference-agreement", type=float)
    parser.add_argument("--max-resnet-top1-drop", type=float)
...
def summarize_resnet(
    frame: pd.DataFrame | None,
    bits: list[int],
    min_images: int,
    *,
    min_logit_cosine: float | None,
    min_reference_agreement: float | None,
    max_top1_drop: float | None,
) -> tuple[pd.DataFrame, list[str]]:
    issues: list[str] = []
    rows: list[dict[str, Any]] = []
    if frame is None:
        return pd.DataFrame(rows), ["missing resnet native quality results"]
    ref = reference_row(frame)
    source_bytes = float(ref["total_model_bytes"]) if ref is not None else None
    reference_top1 = finite_number(ref.get("top1_accuracy")) if ref is not None else None
    native = native_rows(frame)
    for bit in bits:
        match = native[native["bits_per_weight"] == bit]
        if match.empty:
            issues.append(f"missing resnet native row for {bit} bit")
            continue
        row = match.iloc[0]
        images = int(row["images"])
        if images < min_images:
            issues.append(
                f"resnet {bit} bit uses {images} images, below required {min_images}"
            )
        total_bytes = float(row["total_model_bytes"])
        top1 = finite_number(row.get("top1_accuracy"))
        agreement = finite_number(row.get("reference_agreement"))
        logit_cosine = finite_number(row.get("logit_cosine"))
        if top1 is None:
            issues.append(f"resnet {bit} bit has invalid top1_accuracy")
        if agreement is None:
            issues.append(f"resnet {bit} bit has invalid reference_agreement")
        elif (
            min_reference_agreement is not None
            and agreement < min_reference_agreement
        ):
            issues.append(
                f"resnet {bit} bit agreement {agreement:.4f} below "
                f"{min_reference_agreement:.4f}"
            )
        if logit_cosine is None:
            issues.append(f"resnet {bit} bit has invalid logit_cosine")
        elif min_logit_cosine is not None and logit_cosine < min_logit_cosine:
            issues.append(
                f"resnet {bit} bit logit cosine {logit_cosine:.4f} below "
                f"{min_logit_cosine:.4f}"
            )
        top1_delta = (
            top1 - reference_top1
            if top1 is not None and reference_top1 is not None
            else None
        )
        if (
            max_top1_drop is not None
            and top1_delta is not None
            and -top1_delta > max_top1_drop
        ):
            issues.append(
                f"resnet {bit} bit top1 drop {-top1_delta:.4f} above "
                f"{max_top1_drop:.4f}"
            )
        rows.append(
            {
                ...
                "native_backend": scalar(row, "execution_backend"),
                "linear_backend": scalar(row, "level2_linear_backend"),
                "conv_backend": scalar(row, "level2_conv_backend"),
                "spatial_backend": scalar(row, "level2_spatial_backend"),
                "images": images,
                "total_model_bytes": total_bytes,
                "compression_vs_source": (
                    source_bytes / total_bytes if source_bytes else None
                ...
                "top1_accuracy": top1,
                "top1_delta_vs_reference": top1_delta,
                "reference_agreement": agreement,
                "logit_cosine": logit_cosine,
                "logit_mae": scalar(row, "logit_mae"),
                "latency_batch1_ms": scalar(row, "latency_batch1_ms"),
                "images_per_s": scalar(row, "images_per_s"),
            }
        )
    if not native.empty:
        conv_backends = set(native.get("level2_conv_backend", pd.Series(dtype=str)).dropna())
        if "native-cpu" not in conv_backends:
            issues.append("resnet native rows do not use native-cpu conv backend")
    return pd.DataFrame(rows), issues


def summarize_resnet_per_class(
    frame: pd.DataFrame | None, bits: list[int]
) -> tuple[pd.DataFrame, list[str]]:
    issues: list[str] = []
    if frame is None:
        return pd.DataFrame(), ["missing resnet per-class metrics"]
    native = frame[frame["execution_backend"] == "level2-native"]
    if native.empty:
        return frame, ["missing native rows in resnet per-class metrics"]
    for bit in bits:
        bit_rows = native[native["bits_per_weight"] == bit]
        if bit_rows.empty:
            issues.append(f"missing resnet per-class rows for {bit} bit")
            continue
        class_count = int(bit_rows["class_name"].nunique())
        if class_count < 10:
            issues.append(
                f"resnet {bit} bit per-class evidence covers {class_count} classes"
            )
        empty_classes = bit_rows[bit_rows["images"] <= 0]
        if not empty_classes.empty:
            names = ", ".join(empty_classes["class_name"].astype(str).tolist())
            issues.append(f"resnet {bit} bit has empty per-class rows: {names}")
    return frame, issues
...
def main() -> None:
    ...
    resnet_results = args.resnet_results or first_existing(
        level2,
        ["resnet18_dyadic_results.csv"],
    )
    resnet_per_class = args.resnet_per_class or first_existing(
        level2,
        ["resnet18_dyadic_per_class.csv"],
    )
    ...
    resnet_kernels = args.resnet_kernels or first_existing(
        level2,
        ["resnet_native_conv2d.csv", "*conv2d*.csv"],
    )
    ...
    if args.skip_resnet:
        resnet = pd.DataFrame()
        resnet_issues = []
        resnet_per_class_frame = pd.DataFrame()
        resnet_per_class_issues = []
        resnet_kernel = pd.DataFrame()
        resnet_kernel_issues = []
    else:
        resnet, resnet_issues = summarize_resnet(
            read_csv(resnet_results),
            args.bits,
            args.min_resnet_images,
            min_logit_cosine=args.min_resnet_logit_cosine,
            min_reference_agreement=args.min_resnet_reference_agreement,
            max_top1_drop=args.max_resnet_top1_drop,
        )
        resnet_per_class_frame, resnet_per_class_issues = summarize_resnet_per_class(
            read_csv(resnet_per_class), args.bits
        )
        resnet_kernel, resnet_kernel_issues = summarize_kernels(
            read_csv(resnet_kernels), "resnet"
        )
    ...
    issues.extend(resnet_issues)
    issues.extend(resnet_per_class_issues)
    issues.extend(textual_issues)
    issues.extend(qwen_kernel_issues)
    issues.extend(resnet_kernel_issues)
    ...
    resnet.to_csv(output / "resnet_native_evidence.csv", index=False)
    resnet_per_class_frame.to_csv(
        output / "resnet_per_class_evidence.csv", index=False
    )
    ...
    resnet_kernel.to_csv(output / "resnet_kernel_evidence.csv", index=False)
    ...
    payload = {
        ...
        "inputs": {
            ...
            "resnet_results": str(resnet_results) if resnet_results else None,
            "resnet_per_class": str(resnet_per_class) if resnet_per_class else None,
            ...
            "resnet_kernels": str(resnet_kernels) if resnet_kernels else None,
            "textual_audit": str(textual_audit) if textual_audit else None,
        },
        "requested_bits": args.bits,
        "minimum_resnet_images": args.min_resnet_images,
        ...
        "thresholds": {
            ...
            "min_resnet_logit_cosine": args.min_resnet_logit_cosine,
            "min_resnet_reference_agreement": args.min_resnet_reference_agreement,
            "max_resnet_top1_drop": args.max_resnet_top1_drop,
        },
        "textual": textual_evidence,
        "outputs": {
            "qwen": str(output / "qwen_native_evidence.csv"),
            "resnet": str(output / "resnet_native_evidence.csv"),
            "resnet_per_class": str(output / "resnet_per_class_evidence.csv"),
            "textual": str(output / "qwen_textual_native_evidence.csv"),
            "qwen_kernels": str(output / "qwen_kernel_evidence.csv"),
            "resnet_kernels": str(output / "resnet_kernel_evidence.csv"),
